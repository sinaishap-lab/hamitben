import Link from "next/link";
import Image from "next/image";
import { redirect, notFound } from "next/navigation";
import type { LoanStatus, Prisma } from "@prisma/client";
import { auth } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { LoanStatusBadge } from "@/components/loans/LoanStatusBadge";
import { LoanManagerActions } from "./LoanManagerActions";
import { formatDateHe, formatShekel } from "@/lib/utils";

export const dynamic = "force-dynamic";

const TABS: { key: string; label: string; statuses: LoanStatus[] }[] = [
  { key: "PENDING", label: "ממתינות", statuses: ["PENDING"] },
  { key: "APPROVED", label: "אושרו", statuses: ["APPROVED"] },
  { key: "ACTIVE", label: "פעילות", statuses: ["ACTIVE"] },
  {
    key: "HISTORY",
    label: "היסטוריה",
    statuses: ["RETURNED", "OVERDUE", "REJECTED", "CANCELLED"],
  },
];

export default async function GemachLoansPage({
  params,
  searchParams,
}: {
  params: { id: string };
  searchParams: { tab?: string };
}) {
  const session = await auth();
  if (!session?.user) redirect("/login");

  const gemach = await prisma.gemach.findUnique({
    where: { id: params.id },
    select: { id: true, name: true, managerId: true },
  });
  if (!gemach) notFound();

  const isAdmin = session.user.role === "ADMIN";
  const isOwner = gemach.managerId === session.user.id;
  if (!isAdmin && !isOwner) redirect("/");

  const tabKey = TABS.find((t) => t.key === searchParams.tab)?.key ?? "PENDING";
  const tab = TABS.find((t) => t.key === tabKey)!;

  const where: Prisma.LoanWhereInput = {
    tool: { gemachId: gemach.id },
    status: { in: tab.statuses },
  };

  const loans = await prisma.loan.findMany({
    where,
    orderBy: [{ startDate: "desc" }],
    select: {
      id: true,
      status: true,
      startDate: true,
      endDate: true,
      actualReturnDate: true,
      depositAmount: true,
      dailyRate: true,
      totalCharged: true,
      purpose: true,
      adminNotes: true,
      tool: { select: { id: true, name: true, images: true } },
      user: { select: { id: true, name: true, phone: true } },
    },
  });

  const counts: Record<string, number> = {};
  await Promise.all(
    TABS.map(async (t) => {
      counts[t.key] = await prisma.loan.count({
        where: { tool: { gemachId: gemach.id }, status: { in: t.statuses } },
      });
    })
  );

  return (
    <div className="px-4 py-6 flex flex-col gap-4">
      <header>
        <h1 className="text-2xl font-bold text-primary">בקשות והשאלות</h1>
        <p className="text-sm text-text-muted">{gemach.name}</p>
      </header>

      <nav
        className="-mx-4 px-4 flex gap-2 overflow-x-auto scrollbar-thin"
        aria-label="סינון לפי סטטוס"
      >
        {TABS.map((t) => {
          const active = t.key === tabKey;
          return (
            <Link
              key={t.key}
              href={`/gemach/${gemach.id}/loans?tab=${t.key}`}
              className={`shrink-0 px-3 h-9 inline-flex items-center gap-2 rounded-full border text-sm ${
                active
                  ? "bg-primary text-text-inverse border-primary"
                  : "bg-bg-surface text-text border-primary-100"
              }`}
            >
              {t.label}
              <span
                className={`text-xs rounded-full px-1.5 ${
                  active ? "bg-white/20" : "bg-primary-50 text-text-muted"
                }`}
              >
                {counts[t.key] ?? 0}
              </span>
            </Link>
          );
        })}
      </nav>

      {loans.length === 0 ? (
        <p className="text-center text-text-muted py-8">אין השאלות בקטגוריה זו.</p>
      ) : (
        <ul className="flex flex-col gap-3">
          {loans.map((l) => (
            <li
              key={l.id}
              className="bg-bg-surface rounded-2xl border border-primary-100 p-3 flex flex-col gap-2"
            >
              <div className="flex gap-3">
                <div className="relative w-16 h-16 shrink-0 rounded-xl overflow-hidden bg-bg border border-primary-50">
                  {l.tool.images[0] && (
                    <Image
                      src={l.tool.images[0]}
                      alt={l.tool.name}
                      fill
                      sizes="64px"
                      className="object-cover"
                      unoptimized
                    />
                  )}
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="font-bold truncate">{l.tool.name}</div>
                      <div className="text-xs text-text-muted">
                        {l.user.name} ·{" "}
                        <span dir="ltr">{l.user.phone}</span>
                      </div>
                    </div>
                    <LoanStatusBadge status={l.status} />
                  </div>

                  <div className="text-xs text-text-muted mt-1">
                    {formatDateHe(l.startDate)} — {formatDateHe(l.endDate)}
                  </div>
                  {l.totalCharged != null && (
                    <div className="text-xs text-text-muted">
                      חויב {formatShekel(l.totalCharged)}
                    </div>
                  )}
                  {l.purpose && (
                    <div className="text-xs text-text-muted mt-1 italic">
                      “{l.purpose}”
                    </div>
                  )}
                  {l.adminNotes && (
                    <div className="text-xs bg-red-50 text-red-700 rounded-lg px-2 py-1 mt-1">
                      {l.adminNotes}
                    </div>
                  )}
                </div>
              </div>

              <LoanManagerActions loanId={l.id} status={l.status} />
            </li>
          ))}
        </ul>
      )}
    </div>
  );
}
